...
;( function( window ) {
	...
	ElastiStack.prototype._setStackStyle = function() {
		var item1 = this._firstItem(), item2 = this._secondItem(), item3 = this._thirdItem(), item4 = this._fourthItem(), item5 = this._fifthItem();
        var dist = 60;
		...
		if( item2 ) {
			...
			setTransformStyle( item2, is3d ? 'translate3d(60px,0,-60px)' : 'translate(0,0)' );
		}

		if( item3 ) {
			item3.style.opacity = 1;
			item3.style.zIndex = 2;
			setTransformStyle( item3, is3d ? 'translate3d(120px,0,-120px)' : 'translate(0,0)' );
		}
        
        if( item4 ) {
			item4.style.opacity = 1;
			item4.style.zIndex = 1;
			setTransformStyle( item4, is3d ? 'translate3d(-60px,0,-60px)' : 'translate(0,0)' );
		}
        
        if( item5 ) {
			item5.style.opacity = 1;
			item5.style.zIndex = 0;
			setTransformStyle( item5, is3d ? 'translate3d(-120px,0,-120px)' : 'translate(0,0)' );
		}
	};
	...
	ElastiStack.prototype._moveAway = function( instance ) {
		...
		var item2 = this._secondItem(), item3 = this._thirdItem(), item4 = this._fourthItem(), item5 = this._fifthItem();
        ...
        if( item5 ) {
			...
			setTransformStyle( item5, is3d ? 'translate3d(0,0,-240px)' : 'translate(0,0)' );
		}
		...
		var self = this,
			onEndTransFn = function() {
                ...
                if( item5 ) classie.remove( item5, 'move-back' );
	...
	ElastiStack.prototype._onDragStart = function( instance, event, pointer ) {
        ...
        if( item5 ) {
			classie.remove( item5, 'move-back' );
			classie.remove( item5, 'animate' );
		}
	};
	...
	ElastiStack.prototype._onDragMove = function( instance, event, pointer ) {
		if( this._outOfBounds( instance ) ) {
			...
			var item2 = this._secondItem(), item3 = this._thirdItem(), item4 = this._fourthItem(), item5 = this._fifthItem();
			if( item2 ) {
				setTransformStyle( item2, is3d ? 'translate3d(' + ( instance.position.x * .7 ) + 'px,' + ( instance.position.y * .7 ) + 'px, -60px)' : 'translate(' + ( instance.position.x * .7 ) + 'px,' + ( instance.position.y * .7 ) + 'px)' );
			}
			if( item3 ) {
				setTransformStyle( item3, is3d ? 'translate3d(' + ( instance.position.x * .5 ) + 'px,' + ( instance.position.y * .5 ) + 'px, -120px)' : 'translate(' + ( instance.position.x * .5 ) + 'px,' + ( instance.position.y * .5 ) + 'px)' );
			}
            if( item4 ) {
            ...
            if( item5 ) {
				setTransformStyle( item5, is3d ? 'translate3d(' + ( instance.position.x * .1 ) + 'px,' + ( instance.position.y * .1 ) + 'px, -120px)' : 'translate(' + ( instance.position.x * .1 ) + 'px,' + ( instance.position.y * .1 ) + 'px)' );
			}
		}
	};
    ...
    // returns the third item in the stack
	ElastiStack.prototype._fifthItem = function() { 
		if( this.itemsCount >= 5 ) {
			return this.current + 4 < this.itemsCount ? this.items[ this.current + 4 ] : this.items[ Math.abs( this.itemsCount - ( this.current + 4 ) ) ];
		}
	};
